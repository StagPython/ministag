...
"""
    ...
    if not par.is_file():
        print(
            f"Parameter file `{par}` not found, creating one for you.",
            "Modify it to your taste and rerun ministag.",
            sep="\n",
        )
        conf.to_file_(par)
        sys.exit()
    ...
    if not rb2d.conf.numerical.restart and conf.inout.outdir.is_dir():
        print("Output directory already exists.", "Resuming may lead to loss of data.")
